  ...
  defmodule MockWorkflowValidator do
    ...
    def call(conn, _opts) do
      ...
      if workflow_state do
        ...
        valid_path? =
          case {workflow_state.workflow_type, current_step} do
            {:doctor_flow, :list_appointments} ->
              path == "doctor/appointments"

            {:doctor_flow, :access_appointment} ->
              String.match?(path, ~r/doctor\/appointment\/\w+/)

            {:doctor_flow, :fill_medical_details} ->
              String.match?(path, ~r/doctor\/medical-details\/\w+/)

            {:doctor_flow, :record_diagnosis} ->
              String.match?(path, ~r/doctor\/diagnosis\/\w+/)

            {:doctor_flow, :complete_appointment} ->
              String.match?(path, ~r/doctor\/complete\/\w+/)
