...
defmodule DoctorFlowBypassTest do
  ...
  test "doctor flow bypass controller" do
    ...
    _appointment = List.first(mock_data.appointments)
  ...
  def run_test_logic do
    ...
    case WorkflowValidator.validate_medical_details(medical_details) do
      :ok -> IO.puts("Medical details validation passed!")
      error -> IO.puts("Medical details validation failed: #{inspect(error)}")
    end
    ...
    case WorkflowValidator.validate_diagnosis(diagnosis) do
      :ok -> IO.puts("Diagnosis validation passed!")
      error -> IO.puts("Diagnosis validation failed: #{inspect(error)}")
    end

    # Test workflow state transitions
    # Create mock workflow state structs
    scheduled_state = %{workflow_type: :doctor_flow, current_step: :list_appointments}
    in_progress_state = %{workflow_type: :doctor_flow, current_step: :access_appointment}
    ...
    transitions = [
      ...
      {medical_details_state, :record_diagnosis},
      {diagnosis_state, :complete_appointment}
    ]
    ...
    IO.puts("All workflow validations completed!")
    IO.puts("Doctor flow bypass controller is ready for testing.")
    IO.puts("\nTo manually test the doctor flow, you can run:")
    IO.puts("1. Run the isolated tests: ./run_isolated_doctor_tests.sh")
    IO.puts("2. Import the bypass routes in your router.ex file")
