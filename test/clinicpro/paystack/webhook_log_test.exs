defmodule Clinicpro.PaystackWebhookLogTest do
  ...
  describe "WebhookLog schema" do
    test "list/4 returns webhook logs for a specific clinic with filters", %{webhook_log1: webhook_log1} do
    ...
    test "get_with_transaction/2 returns webhook log with transaction for a specific clinic", %{webhook_log1: webhook_log1} do
  ...
  describe "PaystackAdminController webhook actions" do
    ...
    test "webhook_logs/2 handles filtering", %{conn: conn} do
      conn = get(conn, Routes.paystack_admin_path(conn, :webhook_logs, @clinic_1_id, %{"event_type" => "charge.success"}))
      assert html_response(conn, 200) =~ "charge.success"
      
      conn = get(conn, Routes.paystack_admin_path(conn, :webhook_logs, @clinic_1_id, %{"event_type" => "non_existing"}))
      assert html_response(conn, 200) =~ "No webhook logs found"
    end
    
    test "webhook_details/2 shows webhook details for a specific clinic", %{conn: conn, webhook_log1: webhook_log1} do
      conn = get(conn, Routes.paystack_admin_path(conn, :webhook_details, @clinic_1_id, webhook_log1.id))
    ...
    test "webhook_details/2 redirects when webhook not found", %{conn: conn} do
      conn = get(conn, Routes.paystack_admin_path(conn, :webhook_details, @clinic_1_id, "non_existing_id"))
    ...
    test "retry_webhook/2 retries a failed webhook", %{conn: conn, webhook_log2: webhook_log2} do
      with_mock Callback, [retry_webhook: fn(_id, _clinic_id) -> {:ok, webhook_log2} end] do
        conn = get(conn, Routes.paystack_admin_path(conn, :retry_webhook, @clinic_2_id, webhook_log2.id))
        assert redirected_to(conn) == Routes.paystack_admin_path(conn, :webhook_details, @clinic_2_id, webhook_log2.id)
