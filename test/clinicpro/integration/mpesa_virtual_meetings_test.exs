defmodule Clinicpro.Integration.MpesaVirtualMeetingsTest do
  ...
  setup do
    ...
    {:ok, clinic_a} = create_clinic("Clinic A", %{
      mpesa_config: %{
    ...
    {:ok, clinic_b} = create_clinic("Clinic B", %{
      mpesa_config: %{
    ...
    {:ok, clinic_c} = create_clinic("Clinic C", %{
      mpesa_config: %{
    ...
    {:ok, clinic_d} = create_clinic("Clinic D", %{
      mpesa_config: %{
  ...
  describe "M-Pesa multi-tenant tests" do
    ...
    test "transactions are isolated by clinic", %{clinics: clinics} do
      ...
      {:ok, transaction_a} = MPesa.create_transaction(%{
      ...
      {:ok, transaction_b} = MPesa.create_transaction(%{
  ...
  describe "Virtual meetings integration tests" do
    ...
    @tag :external_api
    test "M-Pesa payment triggers meeting creation with correct adapter", %{clinics: clinics, appointments: appointments} do
      ...
      {:ok, _} = MPesa.process_payment_callback(%{
        "ResultCode" => "0",
        "ResultDesc" => "Success",
    ...
    @tag :fallback
    test "system falls back to SimpleAdapter when API fails", %{clinics: clinics, appointments: appointments} do
      # Override clinic A config to use invalid credentials
      {:ok, _} = Config.set_clinic_config(clinics.a.id, %{
        adapter: "Clinicpro.VirtualMeetings.GoogleMeetAdapter",
        google_api_credentials: %{
  ...
  describe "Multi-tenant configuration tests" do
    ...
    test "adapter can be changed at runtime", %{clinics: clinics} do
      ...
      {:ok, _} = Config.set_clinic_adapter(clinics.a.id, Clinicpro.VirtualMeetings.ZoomAdapter)
