defmodule ClinicproWeb.AppointmentHTML do
    ...
    <div class="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-md">
      ...
      <%= if @invoice do %>
        <div class="mb-6 p-4 border rounded-md bg-gray-50">
          ...
          <%= if @invoice.status == "unpaid" do %>
            <div class="mt-4">
              <a
                href={~p"/q/payment/#{@invoice.id}"}
                class="bg-blue-600 text-white py-2 px-4 rounded-md hover:bg-blue-700 inline-block"
              >
                Pay Now
              </a>
            </div>
      ...
      <div class="flex flex-col space-y-4">
        <%= if @_appointment.appointment_type == "virtual" && @invoice && @invoice.status == "paid" do %>
          <a
            href={~p"/q/_appointment/virtual/#{@_appointment.id}"}
            class="bg-green-600 text-white py-2 px-4 rounded-md hover:bg-green-700 text-center"
          >
        ...
        <%= if @_appointment.appointment_type == "onsite" && @invoice && @invoice.status == "paid" do %>
          <a
            href={~p"/q/_appointment/onsite/#{@_appointment.id}"}
            class="bg-green-600 text-white py-2 px-4 rounded-md hover:bg-green-700 text-center"
          >
            View Clinic Details
          </a>
  ...
  def virtual_link(assigns) do
    ...
    <div class="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-md">
      ...
      <div class="mb-6 p-4 border-2 border-green-500 rounded-md bg-green-50">
        ...
        <div class="flex flex-col space-y-4">
          <a
            ...
            class="bg-green-600 text-white py-3 px-4 rounded-md hover:bg-green-700 text-center font-semibold"
          >
      ...
      <div class="flex flex-col space-y-4">
        <a
          href={~p"/q/_appointment/#{@_appointment.id}"}
  ...
  defp appointment_type_badge(type) do
    case type do
      ...
      _unused -> "bg-gray-100 text-gray-800 px-2 py-1 rounded-full text-xs font-semibold"
    end
  end
end
