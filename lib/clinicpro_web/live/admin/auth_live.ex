defmodule ClinicproWeb.Admin.AuthLive do
  @moduledoc """
  LiveView for admin authentication with magic link flow.
  """
  ...
  def handle_event("send_magic_link", %{"email" => email}, socket) do
    case AdminHandler.initiate_magic_link(email, socket.assigns.clinic_id || 1) do
      {:ok, _admin, _token, _magic_link} ->
        {:noreply,
         assign(socket,
  ...
  defp handle_token_validation(token, socket) do
    case AdminHandler.handle_magic_link_login(token, socket.assigns.clinic_id || 1) do
      {:ok, admin} ->
        session_data = %{
          admin_id: admin.id,
          email: admin.email,
