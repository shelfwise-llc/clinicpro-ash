defmodule ClinicproWeb.Patient.AuthLive do
  @moduledoc """
  LiveView for patient authentication with magic link flow.
  """
  ...
  def handle_event("send_magic_link", %{"email" => email}, socket) do
    case PatientHandler.initiate_magic_link(email, socket.assigns.clinic_id || 1) do
      {:ok, _patient, _token, _magic_link} ->
        {:noreply,
         assign(socket,
  ...
  defp handle_token_validation(token, socket) do
    case PatientHandler.handle_magic_link_login(token, socket.assigns.clinic_id || 1) do
      {:ok, patient} ->
        session_data = %{
