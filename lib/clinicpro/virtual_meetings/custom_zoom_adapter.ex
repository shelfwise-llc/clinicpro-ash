defmodule Clinicpro.VirtualMeetings.CustomZoomAdapter do
  ...
  * `{:error, reason}` - On failure, returns an error reason
  ...
  def create_meeting(appointment, opts \\ []) do
    # Get the clinic_id from the appointment
    clinic_id = appointment.clinic_id

    # Get access token for the clinic
    case get_access_token(clinic_id) do
      {:ok, token} ->
        # Create meeting with the token
        create_zoom_meeting(token, appointment, opts)
  ...
  @doc """
  Updates an existing Zoom meeting for an appointment.

  ## Parameters

  * `appointment` - The appointment to update the meeting for
  ...
  * `{:error, reason}` - On failure, returns an error reason
  """
  @impl true
  def update_meeting(appointment, meeting_data, opts \\ []) do
    # Get the clinic_id from the appointment
    clinic_id = appointment.clinic_id
    ...
    case get_access_token(clinic_id) do
      {:ok, token} ->
        ...
        update_zoom_meeting(token, appointment, meeting_data, opts)
  ...
  * `appointment` - The appointment to delete the meeting for
  * `meeting_data` - The existing meeting data
  ...
  * `{:error, reason}` - On failure, returns an error reason
  """
  @impl true
  def delete_meeting(appointment, meeting_data, opts \\ []) do
    # Get the clinic_id from the appointment
    clinic_id = appointment.clinic_id
    ...
    case get_access_token(clinic_id) do
      {:ok, token} ->
        ...
        delete_zoom_meeting(token, meeting_data, opts)
  ...
  defp get_zoom_credentials(clinic_id) do
    ...
    {:ok, %{
      client_id: "clinic_#{clinic_id}_client_id",
      client_secret: "clinic_#{clinic_id}_client_secret",
      account_id: "clinic_#{clinic_id}_account_id"
    }}
  end
  ...
  defp create_zoom_meeting(token, appointment, _opts) do
    ...
    # Format the appointment date and time for Zoom
    start_time = format_appointment_time(appointment)

    # Prepare the request body
    body = Jason.encode!(%{
      topic: "Appointment with Dr. #{get_doctor_name(appointment)}",
    ...
    if is_nil(meeting_id) do
      ...
      case HTTPoison.patch(url, body, headers) do
        {:ok, %{status_code: status}} when status in 200..299 ->
          ...
          # with any updates from the appointment
