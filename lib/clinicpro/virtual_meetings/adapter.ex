defmodule Clinicpro.VirtualMeetings.Adapter do
  ...
  * `{:error, reason}` - On failure, returns an error reason
  """
  @callback create_meeting(appointment :: map(), _opts :: keyword()) ::
              {:ok, map()} | {:error, term()}
  ...
  * `{:error, reason}` - On failure, returns an error reason
  """
  @callback update_meeting(appointment :: map(), _opts :: keyword()) ::
              {:ok, map()} | {:error, term()}
  ...
  * `{:error, reason}` - On failure, returns an error reason
  """
  def create_meeting(appointment, _opts \\ [], clinic_id \\ nil) do
    adapter = get_adapter(clinic_id)
    adapter.create_meeting(appointment, _opts)
  end
  ...
  * `{:error, reason}` - On failure, returns an error reason
  """
  def update_meeting(appointment, _opts \\ [], clinic_id \\ nil) do
    adapter = get_adapter(clinic_id)
    adapter.update_meeting(appointment, _opts)
  end
