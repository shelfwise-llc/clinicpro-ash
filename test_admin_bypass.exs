    ...
    case Doctor.create(doctor_params) do
      {:ok, doctor} ->
        ...
        case Doctor.get(doctor.id) do
          nil -> IO.puts("❌ Failed to get doctor by ID")
          found_doctor -> IO.puts("✅ Found doctor: #{found_doctor.first_name} #{found_doctor.last_name}")
        end
        ...
        case Doctor.update(doctor, update_params) do
          {:ok, updated_doctor} -> IO.puts("✅ Updated doctor bio: #{updated_doctor.bio}")
          {:error, changeset} -> IO.puts("❌ Failed to update doctor: #{inspect(changeset.errors)}")
        end
        
        # Delete doctor
        case Doctor.delete(doctor) do
          {:ok, _} -> IO.puts("✅ Deleted doctor")
          {:error, changeset} -> IO.puts("❌ Failed to delete doctor: #{inspect(changeset.errors)}")
        end
    ...
    case Patient.create(patient_params) do
      {:ok, patient} ->
        ...
        case Patient.get(patient.id) do
          nil -> IO.puts("❌ Failed to get patient by ID")
          found_patient -> IO.puts("✅ Found patient: #{found_patient.first_name} #{found_patient.last_name}")
        end
        ...
        case Patient.update(patient, update_params) do
          {:ok, updated_patient} -> IO.puts("✅ Updated patient medical history: #{updated_patient.medical_history}")
          {:error, changeset} -> IO.puts("❌ Failed to update patient: #{inspect(changeset.errors)}")
        end
        
        # Delete patient
        case Patient.delete(patient) do
          {:ok, _} -> IO.puts("✅ Deleted patient")
          {:error, changeset} -> IO.puts("❌ Failed to delete patient: #{inspect(changeset.errors)}")
        end
    ...
    case Appointment.create(appointment_params) do
      {:ok, appointment} ->
        ...
        case Appointment.update(appointment, update_params) do
          {:ok, updated_appointment} -> IO.puts("✅ Updated appointment status: #{updated_appointment.status}")
          {:error, changeset} -> IO.puts("❌ Failed to update appointment: #{inspect(changeset.errors)}")
        end
        
        # Delete appointment
        case Appointment.delete(appointment) do
          {:ok, _} -> IO.puts("✅ Deleted appointment")
          {:error, changeset} -> IO.puts("❌ Failed to delete appointment: #{inspect(changeset.errors)}")
        end
